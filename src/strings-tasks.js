...
function countVowels(str) {
  const vowelsInStr = str.match(/[aeiouy]/gi);
  return vowelsInStr ? vowelsInStr.length : 0;
}
...
function isPalindrome(str) {
  const cleanedStr = str.replace(/[^a-z0-9]/gi, '').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((result, current) => {
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => {
      return chars.includes(char) ? rot13Chars[chars.indexOf(char)] : char;
    })
    .join('');
}
